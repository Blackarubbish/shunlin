...
func GetMediaList(query dto.MediaQueryDto) (*dto.MediaListResponseDto, error) {
	...
	// 获取总数
	var total int64
	if err := db.Model(&model.Media{}).Count(&total).Error; err != nil {
...
func GetMediaByID(id uint) (*model.Media, error) {
	var media model.Media
	if err := repository.DB.Preload("User").Where("status = ?", "active").First(&media, id).Error; err != nil {
...
func UpdateMediaInfo(id uint, userID uint, description, alt string) (*model.Media, error) {
	var media model.Media
