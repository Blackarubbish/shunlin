...
func CreateCategory(categoryRequest dto.CreateCategoryRequestDto) (*dto.CreateCategoryResponseDto, error) {
	...
	if err := repository.DB.Where("slug = ?", category.Slug).First(&model.Category{}).Error; err == nil {
...
func GetOneByNameOrSlug(name string, slug string) (*model.Category, error) {
	var category model.Category
	if err := repository.DB.Where("name = ? or slug = ?", name, slug).First(&category).Error; err != nil {
...
func GetCategories(query dto.GetCategoriesQueryDto) (*dto.GetCategoriesResponseDto, error) {
	...
	// 构建基础查询
	baseQuery := repository.DB.Model(&model.Category{})
	...
	var postCount int64
	if err := repository.DB.Model(&model.Post{}).Where("category_id = ?", categories[0].ID).Count(&postCount).Error; err != nil {
		return nil, resp.ErrInternal.WithCause(err.Error())
	}
...
func UpdateCategory(id uint, categoryRequest dto.UpdateCategoryRequestDto) (*dto.UpdateCategoryResponseDto, error) {
	...
	if err := repository.DB.Save(&category).Error; err != nil {
...
func DeleteCategory(id uint) error {
	return repository.DB.Transaction(func(tx *gorm.DB) error {
		...
		// 检查是否有关联的文章
		var postCount int64
		if err := tx.Model(&model.Post{}).Where("category_id = ?", id).Count(&postCount).Error; err != nil {
			return resp.ErrInternal.WithCause(err.Error())
		}
